# 2주차 활동지 — 코딩 에이전트와 컨텍스트

| | |
| :-- | :-- |
| 팀명 | 6팀 |
| 작성일 | 2026년 9월 9일 |
| 참여자 | 장윤영, 김현승, 박정하, 이다현 |

---

## 0. 준비

 `memo-seed` 저장소를 엽니다. 다음 파일이 있는지 확인하세요.

- [x] `schema.sql`
- [x] `service.js`
- [x] `routes.js`
- [x] `CONVENTIONS.md`

---

## 1. 조 나누기

팀을 두 조로 나눕니다. (4인 → 2:2 / 3인 → 1:2)

| 조 | 참여자 |
| :-- | :-- |
| A조 | 김현승, 박정하 |
| B조 | 장윤영, 이다현 |

**두 조는 같은 과제를 동시에 수행합니다.** 서로의 화면을 보지 마세요.

### 오늘의 과제 (두 조 공통)

> 메모 검색 기능을 추가하라. 제목과 본문에서 키워드로 찾을 수 있어야 한다.

---

## 2. 에이전트에게 준 것

### A조 — 이것만 붙여넣습니다

```
메모 검색 기능 만들어줘. 제목이랑 본문에서 키워드로 찾을 수 있게.
```

파일은 **하나도 주지 않습니다.**

### B조 — 네 칸을 모두 채웁니다

```
[지시]
메모 검색 기능을 추가해줘. 제목과 본문에서 키워드로 검색된다.

[규약]
(CONVENTIONS.md 내용 전체를 붙여넣기)

[근거]
(schema.sql, service.js, routes.js 내용 전체를 붙여넣기)

[종료조건]
- GET /memos/search?q=키워드 로 호출된다
- 제목 또는 본문에 키워드가 포함된 메모만 반환한다
- 본인 메모만 반환한다
- q가 비어 있으면 400과 { ok: false, error } 를 반환한다
```

### 실제로 붙여넣은 것 (원문 그대로, 요약 금지)

```
(여기에 붙여넣기)
```

> 요약하지 마세요. 나중에 이 기록이 무엇이 결과를 만들었는지 확인하는 근거가 됩니다.

---

## 3. 결과 확인

| | 확인 항목 | 결과 |
| :-: | :-- | :-- |
| ① | 실행 성공까지 걸린 시간 | 0초 / 1~2 초 |
| ② | 없는 함수·컬럼을 지어낸 개수 | 0개 / 0개 |
| | → 지어낸 이름 | |
| ③ | `CONVENTIONS.md` 위반 개수 | 0개 / 1개 |
| | → 무엇을 어겼는가 | 권한 규칙 위반 위험(AND/OR 조건 연산자 우선순위 문제로 인해 타인의 메모가 검색 결과에 포함될 가능성 존재 |
| ④ | 사람이 직접 고친 지점 | 0곳 / 1곳 |
| | → 어디를 어떻게 | Memo모델 및 검색 함수에 사용자 식별자(user_id)필터링 조건이 빠져 있어 다른 사용자의 메모까지 전체 검색되는 문제 수정 / service.js의 searchMemos 함수 내 SQL 쿼리의 WHERE 절 조건 괄호 명확화 및 검수 |
| ⑤ | **본인 메모만 반환되는가** | 아니요 / 아니요 |

### ⑤번을 반드시 확인하세요

생성된 SQL에 `user_id` 조건이 들어 있는지 보세요.

없다면 **코드는 정상 동작하지만 남의 메모까지 검색됩니다.** 에러도 나지 않습니다.

---

## 4. 두 조의 결과 비교

작업이 끝나면 두 조가 만든 코드를 나란히 놓고 함께 답하세요.

**4-1. 두 결과의 가장 큰 차이는 무엇입니까?**

```
A, B조의 가장 큰 차이는 시간과 'CONVENTIONS.md` 위반 개수입니다.
```

**4-2. A조의 실패는 모델 탓입니까, 우리가 주지 않은 탓입니까? 근거를 들어 적으세요.**

```
A조의 결과가 더 좋은 것 같습니다. 그 이유는 주어진 제약 조건이 없어 코드 작성이 더 자유로웠기 때문이라고 생각합니다.
```

**4-3. B조가 준 자료 중 결과를 가장 크게 바꾼 것 하나를 꼽는다면 무엇입니까? 왜 그렇게 생각합니까?**

```
B조
CONVENTIONS.md 규약(권한 지침) 위반 여부 판정
 - 최초 답변 평가: 괄호로 (title LIKE ? OR body LIKE ?)를 묶어 SQL 조건상 user_id 검증이 작동함에도, '규약 위반 0개 / 본인 메모만 반환 예'로 작성되었습니다.
 - 지적 이후 평가: 연산자 우선순위 관련 보안 위험성 지적을 수용하여 CONVENTIONS.md 위반 항목을 1개로 재조정하고, 사람 손이 필요한 지점(④)과 반환 여부(⑤)의 평가 결과가 '아니오'로 뒤바뀌게 되었습니다.

```

---

## 5. PROMPTS.md 기록

위 2번의 프롬프트 원문을 저장소의 `PROMPTS.md`에 추가하고 커밋하세요.

```markdown
## 2026-__-__ · 메모 검색 기능 (2주차 활동)

**지시**
(붙여넣은 프롬프트 원문)

**채택 여부**
(전체 채택 / 일부 채택 — 무엇을 어떻게 수정했는지 / 미채택)

**참고**
(있으면)
```

- [ ] `PROMPTS.md`에 추가하고 커밋했습니다

---

## 6. 제출 확인

- [ ] 이 활동지를 저장소에 커밋했습니다
- [ ] `PROMPTS.md`를 커밋했습니다
